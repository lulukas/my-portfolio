<script lang="ts">
	...
	function handleClick(event: any) {
		goto(`${$page.url.pathname}?lang=${$LL.CHANGE_LANGUAGE.LOCALE()}`, { invalidateAll: true });
	}
</script>

<button on:click={handleClick} class="group flex items-center gap-1 text-base font-extralight">
	<span class="font-extrabold">
		{$LL.CHANGE_LANGUAGE.LANGUAGE()}
	</span>{$LL.CHANGE_LANGUAGE.THIS_WAY()}
	<svg
